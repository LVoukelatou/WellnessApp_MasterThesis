...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      await FirebaseAuth.instance.signInWithEmailAndPassword(
        email: _email.text
            .trim(), // Χρησιμοποιούμε το trim() για να αφαιρέσουμε τυχόν κενά πριν και μετά το email
  ...
  Future<void> _resetPassword() async {
    if (_email.text.trim().isEmpty) {
      setState(() {
        _errorMessage =
            'Γράψε πρώτα το email σου παραπάνω, για να σου στείλουμε σύνδεσμο επαναφοράς.';
      });
      return;
    }
    setState(() {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Σου στείλαμε email με οδηγίες επαναφοράς κωδικού 📩',
            ),
          ),
        );
      }
  ...
  @override
  //κλείνουμε τους controllers όταν το widget καταστρέφεται για να αποφύγουμε διαρροές μνήμης
  void dispose() {
    _email.dispose();
    _password.dispose();
    super.dispose();
